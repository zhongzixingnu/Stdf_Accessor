...
STDF_FILE_ERROR STDF_FILE::read(const char* filename)
{
	...
	if(!in) return STDF_FILE_ERROR::READ_ERROR;

	StdfHeader header;
	STDF_TYPE type = header.read(in);
	if(type != FAR_TYPE) return STDF_FILE_ERROR::FORMATE_ERROR;
    ...
    in.close();
    return STDF_FILE_ERROR::STDF_OPERATE_OK;
}

STDF_FILE_ERROR STDF_FILE::write(const char* filename)
{
	std::ofstream out(filename, std::ios::out );
	if(!out) return STDF_FILE_ERROR::WRITE_ERROR;
